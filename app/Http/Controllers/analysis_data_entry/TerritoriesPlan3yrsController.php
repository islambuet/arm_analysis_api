...
class TerritoriesPlan3yrsController extends RootController
{
    ...
    public function getItem(Request $request): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            ...
            $query=DB::table(TABLE_TERRITORIES_PLAN_3YRS.' as sd');
            $query->select('sd.*');
            $query->where('sd.territory_id','=',$item['territory_id']);
            ...
            $results=DB::table(TABLE_DISTRIBUTORS_SALES.' as sd')
                ...
                ->join(TABLE_DISTRIBUTORS.' as d', 'd.id', '=', 'sd.distributor_id')
                ...
                ->groupBy('year')
                ->where('territories.id','=',$item['territory_id'])
    ...
    public function saveItem(Request $request): JsonResponse
    {
        ...
        $validation_rule['territory_recommendation'] = ['nullable'];
        ...
        $query=DB::table(TABLE_TERRITORIES_PLAN_3YRS.' as sd');
        $query->select('sd.*');
        $query->where('sd.territory_id','=',$itemNew['territory_id']);
        ...
        try {
            $time = Carbon::now();
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_TERRITORIES_PLAN_3YRS;
            ...
            if ($itemId > 0) {
                ...
                DB::table(TABLE_TERRITORIES_PLAN_3YRS)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_TERRITORIES_PLAN_3YRS)->insertGetId($itemNew);
