...
class DistributorsSalesReturnController extends RootController
{
    public $api_url = 'analysis_data_entry/distributors_sales_return';
    public $permissions;
    ...
    public function saveItem(Request $request): JsonResponse
    {
        ...
        if ($itemId > 0) {
            $result = DB::table(TABLE_DISTRIBUTORS_SALES_RETURN)->select(array_keys($validation_rule))->find($itemId);
            if (!$result) {
                return response()->json(['error' => 'ITEM_NOT_FOUND', 'messages' => __('Invalid Id ' . $itemId)]);
            }
...
//                    }
        ...
        try {
            $time = Carbon::now();
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_DISTRIBUTORS_SALES_RETURN;
            ...
            if ($itemId > 0) {
                ...
                DB::table(TABLE_DISTRIBUTORS_SALES_RETURN)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_DISTRIBUTORS_SALES_RETURN)->insertGetId($itemNew);
    ...
    public function saveItems(Request $request): JsonResponse
    {
        ...
        $id_start=$id_end=1;
        $result = DB::table(TABLE_DISTRIBUTORS_SALES_RETURN)->select('id')->orderBy('id','DESC')->first();
        if($result){
        ...
        try {
            ...
            foreach ($itemsNew as $row){
                ...
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $id_end = DB::table(TABLE_DISTRIBUTORS_SALES_RETURN)->insertGetId($itemNew);
            }
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_DISTRIBUTORS_SALES_RETURN;
            $dataHistory['controller'] = (new \ReflectionClass(__CLASS__))->getShortName();
