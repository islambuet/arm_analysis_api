...
class LocationAreasController extends RootController
{
    public $api_url = 'setup/location_areas';
    public $permissions;
    ...
    public function initialize(): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            $location_parts = DB::table(TABLE_LOCATION_PARTS)
                ...
                ->get();
            return response()->json(['error'=>'','permissions'=>$this->permissions,'hidden_columns'=>TaskHelper::getHiddenColumns($this->api_url,$this->user),'location_parts'=>$location_parts]);
        } else {
    ...
    public function getItems(Request $request): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            ...
            $query=DB::table(TABLE_LOCATION_AREAS.' as areas');
            $query->select('areas.*');
            ...
            $query->where('areas.status', '!=', SYSTEM_STATUS_DELETE);//
            if ($perPage == -1) {
                $perPage = $query->count();
                if($perPage<1){
                    $perPage=50;
    ...
    public function saveItem(Request $request): JsonResponse
    {
        ...
        $validation_rule['part_id'] = ['required','numeric'];
        ...
        if ($itemId > 0) {
            $result = DB::table(TABLE_LOCATION_AREAS)->select(array_keys($validation_rule))->find($itemId);
            if (!$result) {
                return response()->json(['error' => 'ITEM_NOT_FOUND', 'messages' => __('Invalid Id ' . $itemId)]);
            }
...
//                    }
        ...
        try {
            $time = Carbon::now();
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_LOCATION_AREAS;
            ...
            if ($itemId > 0) {
                ...
                DB::table(TABLE_LOCATION_AREAS)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_LOCATION_AREAS)->insertGetId($itemNew);
