...
class TypeMonthsController extends RootController
{
    public $api_url = 'setup/type_months';
    public $permissions;
    ...
    public function getItems(Request $request): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            ...
            $query=DB::table(TABLE_TYPE_MONTHS.' as sd');
            $query->select('sd.*');
            $query->join(TABLE_LOCATION_TERRITORIES.' as territories', 'territories.id', '=', 'sd.territory_id');
            ...
            $query->join(TABLE_CROP_TYPES.' as crop_types', 'crop_types.id', '=', 'sd.type_id');
    ...
    public function saveItem(Request $request): JsonResponse
    {
        ...
        if ($itemId > 0) {
            $result = DB::table(TABLE_TYPE_MONTHS)->select(array_keys($validation_rule))->find($itemId);
            if (!$result) {
                return response()->json(['error' => 'ITEM_NOT_FOUND', 'messages' => __('Invalid Id ' . $itemId)]);
            }
...
//                    }
        ...
        try {
            ...
            if ($itemId > 0) {
                ...
                DB::table(TABLE_TYPE_MONTHS)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_TYPE_MONTHS)->insertGetId($itemNew);
