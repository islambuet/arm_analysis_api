...
class IncentiveSlabsController extends RootController
{
    public $api_url = 'setup/incentive_slabs';
    public $permissions;
    ...
    {
        if ($this->permissions->action_0 == 1) {
            $result = DB::table(TABLE_INCENTIVE_SLABS)->find($itemId);
            if (!$result) {
    ...
    public function saveItem(Request $request): JsonResponse
    {
        ...
        if ($itemId > 0) {
            $result = DB::table(TABLE_INCENTIVE_SLABS)->select(array_keys($validation_rule))->find($itemId);
            if (!$result) {
                return response()->json(['error' => 'ITEM_NOT_FOUND', 'messages' => __('Invalid Id ' . $itemId)]);
            }
        ...
        try {
            $time = Carbon::now();
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_INCENTIVE_SLABS;
            ...
            if ($itemId > 0) {
                ...
                DB::table(TABLE_INCENTIVE_SLABS)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_INCENTIVE_SLABS)->insertGetId($itemNew);
