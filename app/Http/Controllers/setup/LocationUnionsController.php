...
class LocationUnionsController extends RootController
{
    public $api_url = 'setup/location_unions';
    public $permissions;
    ...
    public function getItems(Request $request): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            ...
            $query=DB::table(TABLE_LOCATION_UNIONS.' as unions');
            $query->select('unions.*');
            $query->join(TABLE_LOCATION_UPAZILAS.' as upazilas', 'upazilas.id', '=', 'unions.upazila_id');
            ...
            $query->orderBy('unions.id', 'DESC');
            $query->where('unions.status', '!=', SYSTEM_STATUS_DELETE);//
            if ($perPage == -1) {
                $perPage = $query->count();
                if($perPage<1){
                    $perPage=50;
    ...
    public function getItem(Request $request, $itemId): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            ...
            $query->addSelect('upazilas.name as upazila_name');
            $query->join(TABLE_LOCATION_DISTRICTS.' as districts', 'districts.id', '=', 'upazilas.district_id');
            $query->addSelect('districts.name as district_name','districts.id as district_id');

            $query->where('unions.id','=',$itemId);
            $result = $query->first();
            if (!$result) {
    ...
    public function saveItem(Request $request): JsonResponse
    {
        ...
        $validation_rule['upazila_id'] = ['required','numeric'];
        ...
        if ($itemId > 0) {
            $result = DB::table(TABLE_LOCATION_UNIONS)->select(array_keys($validation_rule))->find($itemId);
            if (!$result) {
                return response()->json(['error' => 'ITEM_NOT_FOUND', 'messages' => __('Invalid Id ' . $itemId)]);
            }
...
//                    }
        ...
        try {
            $time = Carbon::now();
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_LOCATION_UNIONS;
            ...
            if ($itemId > 0) {
                ...
                DB::table(TABLE_LOCATION_UNIONS)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_LOCATION_UNIONS)->insertGetId($itemNew);
