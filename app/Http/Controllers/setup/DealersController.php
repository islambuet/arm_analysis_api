...
class DealersController extends RootController
{
    public $api_url = 'setup/dealers';
    public $permissions;
    ...
    public function initialize(): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            ...
            $response['distributors'] = DB::table(TABLE_DISTRIBUTORS)
                ...
                ->where('status', SYSTEM_STATUS_ACTIVE)
                ->get();
            $response['dealer_types'] = DB::table(TABLE_DEALER_TYPES)
                ->select('id', 'name')
    ...
    public function getItems(Request $request): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            ...
            $query=DB::table(TABLE_DEALERS.' as dealers');
            ...
            $query->join(TABLE_DEALER_TYPES.' as dealer_types', 'dealer_types.id', '=', 'dealers.dealer_type_id');
            $query->addSelect('dealer_types.name as dealer_type_name');
            $query->orderBy('dealers.id', 'DESC');
            $query->where('dealers.status', '!=', SYSTEM_STATUS_DELETE);
            if ($perPage == -1) {
                $perPage = $query->count();
                if($perPage<1){
                    $perPage=50;
    ...
    public function saveItem(Request $request): JsonResponse
    {
        ...
        $validation_rule['dealer_type_id'] = ['required','numeric'];
        ...
        if ($itemId > 0) {
            $result = DB::table(TABLE_DEALERS)->select(array_keys($validation_rule))->find($itemId);
            if (!$result) {
                return response()->json(['error' => 'ITEM_NOT_FOUND', 'messages' => __('Invalid Id ' . $itemId)]);
            }
...
//                    }
        ...
        try {
            $time = Carbon::now();
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_DEALERS;
            ...
            if ($itemId > 0) {
                ...
                DB::table(TABLE_DEALERS)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_DEALERS)->insertGetId($itemNew);
