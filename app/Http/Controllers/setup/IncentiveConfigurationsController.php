...
class IncentiveConfigurationsController extends RootController
{
    public $api_url = 'setup/incentive_configurations';
    public $permissions;
    ...
    public function getItems(Request $request): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            ...
            $query=DB::table(TABLE_INCENTIVE_CONFIGURATIONS);
    ...
    {
        if ($this->permissions->action_0 == 1) {
            $result = DB::table(TABLE_INCENTIVE_CONFIGURATIONS)->find($itemId);
            if (!$result) {
    ...
    public function saveItem(Request $request): JsonResponse
    {
        ...
        if ($itemId > 0) {
            $result = DB::table(TABLE_INCENTIVE_CONFIGURATIONS)->select(array_keys($validation_rule))->find($itemId);
            if (!$result) {
                return response()->json(['error' => 'ITEM_NOT_FOUND', 'messages' => __('Invalid Id ' . $itemId)]);
            }
            ...
            $result = DB::table(TABLE_INCENTIVE_CONFIGURATIONS)->where('fiscal_year',$itemNew['fiscal_year'])->where('purpose',$itemNew['purpose'])->first();
            if ($result) {
                return response()->json(['error' => 'ITEM_EXISTS', 'messages' => __('Configuration Exits. Please Edit')]);
            }
        }
        //if itemNew Empty
        if (!$itemNew) {
        ...
        try {
            $time = Carbon::now();
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_INCENTIVE_CONFIGURATIONS;
            ...
            if ($itemId > 0) {
                ...
                DB::table(TABLE_INCENTIVE_CONFIGURATIONS)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_INCENTIVE_CONFIGURATIONS)->insertGetId($itemNew);
